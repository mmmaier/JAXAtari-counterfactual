...
# Frostbite-specific layout constants (matches FrostbiteConstants defaults)
_ICE_ROW_Y    = (98, 122, 147, 172)  # y-position of each of the 4 ice rows
_ICE_BLOCK_H  = 8                    # sprite height for both narrow and wide blocks
_ICE_SENTINEL = -512                 # unused-block marker in ice_block_positions
_IGLOO_X      = 154                  # fixed igloo x (FrostbiteConstants.IGLOO_X)
_IGLOO_Y      = 44                   # fixed igloo y (FrostbiteConstants.IGLOO_Y)
_IGLOO_W      = 16                   # approximate igloo sprite width
_IGLOO_H      = 24                   # approximate igloo sprite height


def objects_from_obs(obs, state=None) -> list:
    """
    Extract all game objects for Frostbite.

    From obs (ObjectObservation fields): bailey, obstacles (fish/crabs), bear.
    From state (when provided): individual ice blocks and igloo.
    Handles both single objects (scalar arrays) and batches (1-D arrays).
    """
    objs = []

    # --- ObjectObservation fields: bailey, obstacles, bear ---
    ...
    # --- Ice blocks from state ---
    ice_positions = np.array(state.ice_block_positions)  # shape (4, 6)
    ice_counts    = np.array(state.ice_block_counts)      # shape (4,)
    for row in range(4):
        count = int(ice_counts[row])
        # narrow layout (6 blocks) -> width 12; wide layout (3 blocks) -> width 24
        block_w = 12 if count == 6 else 24
        row_y   = _ICE_ROW_Y[row]
        for col in range(count):
            bx = int(ice_positions[row, col])
            if bx == _ICE_SENTINEL:
                continue
            objs.append({"category": "IceBlock", "x": bx, "y": row_y,
                         "w": block_w, "h": _ICE_BLOCK_H})

    # --- Igloo from state ---
    igloo_idx = int(state.building_igloo_idx)
    if igloo_idx >= 0:
        objs.append({"category": "Igloo", "x": _IGLOO_X, "y": _IGLOO_Y,
                     "w": _IGLOO_W, "h": _IGLOO_H})
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Play Frostbite, save frames and dataset.",
        allow_abbrev=False,
    )
    parser.add_argument("-g", "--game", type=str, required=True,
                        help="Name of the game (use 'frostbite').")
    ...
    while running:
        ...
        if not frame_by_frame or next_frame_asked:
            ...
            if step_idx % save_stride == 0:
                prev_objs = objects_from_obs(prev_obs, prev_state)
                next_objs = objects_from_obs(next_obs, state)
                ...
                row = {
                    ...
                    "done": bool(done),
                    "temperature": int(np.array(prev_obs.temperature)),
                }
                rows.append(row)
    ...
    if rows:
        ...
        df = pd.DataFrame(rows, columns=[
            ...
            "action", "reward", "original_reward", "done", "temperature",
        ])
